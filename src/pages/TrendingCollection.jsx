...
const TrendingCollection = () => {
  ...
  const fetchTrendingScents = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const params = {
      ...
      const response = await ScentService.getTrendingScents(params);
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to fetch trending scents');
        setScents([]);
      }
    } catch (err) {
      console.error('Error fetching trending scents:', err);
      setError('Failed to load trending scents');
      setScents([]);
    } finally {
      setLoading(false);
    }
          ...
          <div className="max-w-7xl mx-auto">
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <button
                  onClick={fetchTrendingScents}
                  className="bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300"
                >
                  Try Again
                </button>
              </div>
              ...
              <>
                {/* <div className="mb-6 text-center">
                  <p className="text-gray-600 dark:text-gray-400">
                    Showing {scents.length} of {totalPages * itemsPerPage} trending scents
                  </p>
                </div> */}
